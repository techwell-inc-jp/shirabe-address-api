 ...
 *
 * 記録スキーマ(暦 API と同形状に加えて、住所 API 固有フィールドを追加):
 *   blobs(順序固定、暦 API と共通部分):
 ...
 *     9: Response type         success/ambiguous/error/(空) — 実装指示書 §7.2 blob8
 *    10: Coverage status       in_coverage/out_of_coverage/(空) — 実装指示書 §7.2 blob10
 *   doubles:
 *     0: HTTP status
